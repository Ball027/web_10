...
router.get("/blogs/:id", function (req, res, next) {
  ...
  const promise2 = pool.query("SELECT c.*,i.file_path FROM comments as c LEFT JOIN images as i ON(c.id=i.comment_id) WHERE c.blog_id=? ORDER BY c.id asc", [
    req.params.id,
  ]);
